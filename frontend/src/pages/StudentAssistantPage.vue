...
const timelineOpen = reactive({}); // 每条消息的时间轴折叠状态，key=消息在messages中的idx
...
const renderedMessages = computed(() =>
  messages.value.map((m) => ({
    ...
    streaming: m._streaming || false,
  })),
);

onMounted(async () => {
  await loadThreads();
  const saved = sessionStorage.getItem("assistant_thread_id");
  if (saved && threads.value.find((t) => t.id === saved)) selectThread(saved);
  fetchSuggestions();
});
onBeforeUnmount(() => stopStreaming());

async function loadThreads() {
  try { const res = await listThreads("student"); threads.value = res.data.threads || []; } catch { /* */ }
}
async function handleNewThread() {
  try {
    const res = await createThread("student");
    threads.value.unshift({ id: res.data.id, title: res.data.title || "新对话" });
    await selectThread(res.data.id);
  } catch { ElMessage.error("创建会话失败"); }
}
async function handleDeleteThread(threadId) {
  try {
    await ElMessageBox.confirm("确定要删除吗？", "删除确认", { confirmButtonText: "删除", cancelButtonText: "取消", type: "warning" });
    await deleteThread(threadId);
    threads.value = threads.value.filter((t) => t.id !== threadId);
    if (currentThreadId.value === threadId) {
      messages.value = []; currentThreadId.value = null; threadTitle.value = "新对话";
      sessionStorage.removeItem("assistant_thread_id");
      if (threads.value.length > 0) await selectThread(threads.value[0].id);
    }
  } catch { /* */ }
}
async function selectThread(threadId) {
  currentThreadId.value = threadId;
  sessionStorage.setItem("assistant_thread_id", threadId);
  threadTitle.value = threads.value.find((t) => t.id === threadId)?.title || "新对话";
  messages.value = [];
  // 从后端加载对话历史
  try {
...
async function handleSendText() {
  ...
  try {
    ...
    const resp = await fetch("/api/v1/assistants/student/chat/stream", {
      method: "POST", headers,
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (evt.type === "content") {
            ...
            messages.value[aiIdx].toolCalls.push({ name: evt.tool, status: "running", startTime: Date.now() });
          } else if (evt.type === "tool_end") {
            const tc = messages.value[aiIdx].toolCalls;
            for (let i = tc.length - 1; i >= 0; i--) {
              if (tc[i].name === evt.tool && tc[i].status === "running") {
                tc[i].status = "done"; tc[i].endTime = Date.now(); tc[i].output = evt.output; break;
              }
            }
          } else if (evt.type === "thread_id") {
            if (!currentThreadId.value) { currentThreadId.value = evt.thread_id; sessionStorage.setItem("assistant_thread_id", evt.thread_id); }
    ...
    if (e.name !== "AbortError") {
      ...
      messages.value[aiIdx].content = messages.value[aiIdx].content || "抱歉，连接出现问题，请稍后再试。";
    }
  } finally {
    loading.value = false;
...
function stopStreaming() {
  if (abortController.value) { abortController.value.abort(); abortController.value = null; loading.value = false; }
}
function scrollToBottom() {
  nextTick(() => { if (chatContainer.value) chatContainer.value.scrollTop = chatContainer.value.scrollHeight; });
}
async function fetchSuggestions() {
  const cacheKey = "assistant_suggestions_student";
  ...
  try {
    const token = localStorage.getItem("access_token");
    const resp = await fetch("/api/v1/assistants/student/suggestions", {
      method: "POST",
...
function refreshSuggestions() {
  sessionStorage.removeItem("assistant_suggestions_student");
  suggestions.value = [];
  fetchSuggestions();
}
...
// 拦截 Markdown 中的 clip:// 链接，使用 Vue Router 跳转
function handleChatClick(e) {
  ...
  if (href && href.startsWith("clip://")) {
    e.preventDefault();
    const clipId = href.replace("clip://", "");
    router.push(`/student/clips/${clipId}`);
  }
}

function copyContent(msg) {
  navigator.clipboard.writeText(msg.content).then(() => ElMessage.success("已复制"));
}
...
async function submitEdit() {
  ...
  messages.value.splice(editingIdx.value);
  const text = editText.value;
  cancelEdit();
  inputText.value = text;
  await handleSendText();
}
async function regenerateMsg(aiIdx) {
  let userIdx = -1;
  for (let i = aiIdx - 1; i >= 0; i--) { if (messages.value[i].role === "user") { userIdx = i; break; } }
  if (userIdx < 0) return;
...
// 格式化耗时
function formatMs(start, end) { return end && start ? ((end - start) / 1000).toFixed(1) + "s" : ""; }
  ...
  <div class="assistant-page">
    ...
    <aside class="sidebar" :class="{ collapsed: !sidebarOpen }">
      <div class="sidebar-hd">
        <el-button :icon="Plus" size="small" type="primary" @click="handleNewThread">新建对话</el-button>
      </div>
      <div class="thread-list">
        <div
          v-for="t in threads" :key="t.id"
          class="thread-item" :class="{ active: t.id === currentThreadId }"
    ...
    <main class="chat-main">
      ...
      <div ref="chatContainer" class="chat-body" @click="handleChatClick">
        <div v-if="messages.length === 0 && !loading" class="chat-empty">
          <div class="empty-icon"><ChatDotRound style="font-size:32px;color:#58CC02" /></div>
          <h3>学习助手</h3>
          <p>帮你查看学习统计、分析错题、推荐影视片段</p>
            ...
            <div class="msg-bubble">
              ...
              <div v-else-if="editingIdx === idx" class="edit-area">
                ...
                <div class="edit-btns">
                  ...
                  <el-button size="small" type="primary" @click="submitEdit">保存并重发</el-button>
                </div>
              </div>
...
.sidebar { width: 260px; min-width: 260px; display: flex; flex-direction: column; background: linear-gradient(180deg, #F8FAF6 0%, #F2F5EE 100%); border-right: 1px solid rgba(88,204,2,.1); transition: all .3s cubic-bezier(.4,0,.2,1); overflow: hidden; }
...
.thread-item::before { content: ''; position: absolute; left: 0; top: 50%; transform: translateY(-50%); width: 3px; height: 0; background: var(--color-primary, #58CC02); border-radius: 0 3px 3px 0; transition: height .2s; }
.thread-item:hover { background: rgba(255,255,255,.7); }
.thread-item.active { background: #fff; box-shadow: 0 1px 4px rgba(88,204,2,.1); }
.thread-item.active::before { height: 60%; }
.t-icon { color: rgba(75,75,75,.3); font-size: 15px; flex-shrink: 0; transition: color .2s; }
.thread-item.active .t-icon { color: var(--color-primary, #58CC02); }
...
.hd-toggle:hover { color: var(--color-primary, #58CC02); background: rgba(88,204,2,.05); }
.hd-title { font-size: 14px; font-weight: 600; color: #333; }

.chat-body { flex: 1; overflow-y: auto; padding: 20px 24px; background: radial-gradient(ellipse at 50% 0%, rgba(88,204,2,.012) 0%, transparent 60%), #fff; }
.chat-empty { display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; text-align: center; }
.chat-empty .empty-icon { width: 80px; height: 80px; border-radius: 20px; background: linear-gradient(135deg, rgba(88,204,2,.08), rgba(88,204,2,.02)); display: flex; align-items: center; justify-content: center; margin-bottom: 20px; animation: float 3s ease-in-out infinite; }
...
.role-ai .msg-avatar { background: linear-gradient(135deg, #58CC02, #4CAF00); color: #fff; }
.role-user .msg-avatar { background: linear-gradient(135deg, #1CB0F6, #0E9DE0); color: #fff; }

.msg-bubble { max-width: 100%; padding: 12px 16px; border-radius: 16px; font-size: 14.5px; line-height: 1.75; letter-spacing: .01em; }
.role-ai .msg-bubble { background: #F5F5F7; color: #3A3A3A; border-bottom-left-radius: 6px; box-shadow: 0 1px 3px rgba(0,0,0,.04); }
.role-user .msg-bubble { background: linear-gradient(135deg, #58CC02, #4CAF00); color: #fff; border-bottom-right-radius: 6px; box-shadow: 0 2px 8px rgba(88,204,2,.25); }
...
.act-btn:hover { color: var(--color-primary, #58CC02); background: rgba(88,204,2,.06); }
.role-user .act-btn { color: rgba(75,75,75,.3); }
.role-user .act-btn:hover { color: var(--color-primary, #58CC02); }
...
.chat-input :deep(.el-textarea__inner:focus) { border-color: var(--color-primary, #58CC02) !important; background: #fff !important; box-shadow: 0 0 0 3px rgba(88,204,2,.08) !important; }
...
.markdown-body :deep(blockquote) { margin: 10px 0; padding: 6px 14px; border-left: 3px solid #58CC02; background: rgba(88,204,2,.05); border-radius: 0 6px 6px 0; color: #6B6B6B; }
...
.markdown-body :deep(th) { background: #F5F5F7; border: 1px solid #E8E8EC; padding: 10px 14px; text-align: left; font-weight: 600; color: #555; }
.markdown-body :deep(td) { border: 1px solid #E8E8EC; padding: 10px 14px; }
.markdown-body :deep(tr:nth-child(even) td) { background: rgba(0,0,0,.015); }

/* ════ Tool Timeline ════ */
.tool-timeline { margin-bottom: 14px; border: 1px solid rgba(88,204,2,.12); border-radius: 10px; overflow: hidden; font-size: 12.5px; background: rgba(88,204,2,.015); }
.tl-header { display: flex; align-items: center; gap: 8px; padding: 9px 14px; background: rgba(88,204,2,.04); cursor: pointer; user-select: none; transition: background .15s; }
.tl-header:hover { background: rgba(88,204,2,.08); }
.tl-caret { font-size: 10px; color: rgba(75,75,75,.35); width: 14px; transition: transform .2s; }
.tl-title { font-weight: 600; color: #4A7A30; font-size: 12px; letter-spacing: .03em; text-transform: uppercase; }
.tl-count { font-size: 11px; color: rgba(75,75,75,.4); margin-left: auto; font-variant-numeric: tabular-nums; }
.tl-body { border-top: 1px solid rgba(88,204,2,.08); }
...
.tl-item.done .tl-dot { background: linear-gradient(135deg, #E5F8D1, #D2F0B8); color: #4CAF00; font-size: 11px; font-weight: 700; box-shadow: 0 1px 3px rgba(88,204,2,.15); }
.tl-item:not(.done) .tl-dot { background: linear-gradient(135deg, #DDF4FF, #C8EAFF); box-shadow: 0 1px 4px rgba(28,176,246,.2); }
...
.tl-dot .spin { display: inline-block; width: 11px; height: 11px; border: 2px solid rgba(28,176,246,.2); border-top-color: #1CB0F6; border-radius: 50%; animation: spin .7s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }

/* ════ Suggestions ════ */
.suggestion-loading { margin-top: 28px; display: grid; grid-template-columns: 1fr 1fr; gap: 10px; max-width: 420px; width: 100%; }
.s-skel { height: 44px; border-radius: 14px; background: linear-gradient(90deg, #f0f0f0 25%, #e8e8e8 50%, #f0f0f0 75%); background-size: 200% 100%; animation: shimmer 1.5s infinite; }
...
.suggestions-refresh:hover { color: #58CC02; background: rgba(88,204,2,.06); }
...
.suggestion-chip {
  ...
  border: 1px solid rgba(88,204,2,.10);
  color: #4A7A30;
  font-size: 13px;
  cursor: pointer;
...
.suggestion-chip:hover {
  ...
  box-shadow: 0 4px 14px rgba(88,204,2,.12);
