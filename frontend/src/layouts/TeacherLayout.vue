...
async function fetchUnread() {
  try {
    const res = await request.get("/teacher/communication/unread-count");
    userStore.unreadCount = res.data?.messages || res?.messages || 0;
  } catch { /* */ }
}
  ...
  <div class="app-layout">
    <header class="app-header">
      <div class="header-left">
        <router-link to="/teacher" class="app-logo">
          <span class="logo-icon">R</span>RealEnglish 教师端
        </router-link>
      </div>
      <div class="header-right">
        <router-link to="/teacher" class="nav-link">管理看板</router-link>
        <router-link to="/teacher/students" class="nav-link">学生管理</router-link>
        <router-link to="/teacher/communication" class="nav-link">
          学生消息
          <el-badge v-if="userStore.unreadCount > 0" :value="userStore.unreadCount" class="nav-badge" />
        </router-link>
        <router-link to="/teacher/tasks" class="nav-link">任务公告</router-link>
        <router-link to="/teacher/assistant" class="nav-link nav-cta">AI助手</router-link>
        <span class="nav-link nav-user" @click="showProfile = true">
          {{ userStore.userInfo?.username || "个人中心" }}
        </span>
        <el-button size="small" text @click="handleLogout" style="color:rgba(255,255,255,.7)">退出</el-button>
      </div>
    </header>
    <main class="app-main">
      <router-view />
